...
def run_scan(music_dir, verbose=False):
    ...
    # Load discord map
    discord_map = {}
    map_path = os.path.join(DATA_DIR, "artist_discord_map.json")
    if os.path.exists(map_path):
        try:
            with open(map_path, "r") as f:
                discord_map = json.load(f).get("artists", {})
        except Exception as e:
            print(f"Warning: Could not load artist_discord_map.json: {e}")

    # Add canonical_artists and discord_users to each track
    for p in playlists:
        for t in p.get("tracks", []):
            if t.get("artist"):
                canonical_artists = collab_utils.get_canonical_artists_for_track(t["artist"], raw_artist_counts)
                t["canonical_artists"] = canonical_artists
                
                discord_users = []
                for ca in canonical_artists:
                    entry = discord_map.get(ca)
                    if entry and entry.get("handles"):
                        discord_users.append(entry["handles"][0].lower())
                    else:
                        discord_users.append(ca)
                
                t["discord_users"] = list(set(discord_users))
